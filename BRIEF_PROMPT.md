...
- 업종지수를 못 구하면 그 섹터 대표 종목 2~3개 평균으로 계산하고 "(대표종목 기준)" 표기한다. 필수 섹터에 "미확인", 공란, 범위값을 쓰면 발행 검증에서 실패한다.
...
- 수치와 사실은 지어내지 않는다. 선택 항목의 사실을 확인할 수 없으면 해당 선택 항목을 빼고, 지수, 섹터, 핵심 지표 같은 필수 항목은 공신력 있는 대체 출처나 대표 종목 계산으로 정확한 값을 채운다. 끝내 확인할 수 없으면 부정확한 브리핑을 발행하지 않는다.
